/*  =========================================================================
    fty_example_selftest.c - run selftests
...
// Tests for draft public classes:
    { "fty_example_server", fty_example_server_test, false, true, NULL },
...
{
    test_item_t *item;
    printf ("Running fty-example selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_EXAMPLE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_example_private_selftest (verbose, item->subtest);
#endif // FTY_EXAMPLE_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_example_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-example test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_EXAMPLE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_example_private_selftest (verbose, test->subtest);
#endif // FTY_EXAMPLE_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
